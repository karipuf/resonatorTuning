...
ap.add_argument('--lr',help='Initial learning rate (default: sample from [0.01,0.005,0.001,0.0005])')
...
def sdecay(ep,initRate):
    return initRate*0.5**int(ep/800)

pSamper=iter(ParameterSampler(pDict,n_iter=nParams,random_state=ProcessArg(parsed.rs,10)))
for count in range(nParams):
    
    pVec=next(pSamper)
    rateschedule=keras.callbacks.LearningRateScheduler(lambda ep: pVec['lr']*0.5**(int(ep/500.)),verbose=1)
